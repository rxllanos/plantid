  ...
  if (info === 'Plant_Information') {
    try {
      ...
      if(data1.health_is_healthy_binary===false){
        document.getElementById("diseaseButton").style.display="block";
        diseasefunction(id);
        
      }
      else{
        document.getElementById("diseaseButton").style.display="none";
      ...
      ingredients.forEach((ingredient) => {
        ...
        card.innerHTML = `
            <div class="card-body">
                ...
                <button id="modal2" class="btn btn-primary" data-bs-target="#exampleModalToggle2" data-bs-toggle="modal">Nutrition info + Recepies!</button>
            </div>
        `;
...
async function diseasefunction(id) {
  try {
